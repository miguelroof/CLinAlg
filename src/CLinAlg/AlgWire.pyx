...
cdef bint isClosed(double * wirePoint, int * indexes, int numpoints):
    if indexes[0] == indexes[numpoints - 1]:
        ...
        return all([fabs(wirePoint[indexes[0] * 3 + i] - wirePoint[indexes[numpoints - 1] * 3 + i]) < presition for i in
                    range(3)])
...
cdef bint getPointFromOrigin(double * toPoint, double * wirePoint, int * indexes, unsigned int numpoints,
                             double length):
    cdef unsigned int i, j
    cdef double tlen = 0
    ...
    for i in range(<unsigned int> (numpoints - 1)):
        ...
        if slen and tlen + slen >= length:
            ...
            for j in range(3):
                toPoint[j] = wirePoint[indexes[i] * 3 + j] + prop * (
                            wirePoint[indexes[i + 1] * 3 + j] - wirePoint[indexes[i] * 3 + j])
            return True
        tlen += slen
    return False
...
cdef bint getDirectionAtPoint(double * toDir, double * wirePoint, int * indexes, unsigned int numpoints,
                              double * point):
    cdef int i
    for i in range(numpoints - 1):
        if AlgVector.isEqual(&wirePoint[indexes[i] * 3], point) or AlgSegment.isInside(&wirePoint[indexes[i] * 3],
                                                                                       &wirePoint[indexes[i + 1] * 3],
                                                                                       point, <bint> False):
...
cdef bint isClockWise(double * wirePoint, int * indexes, unsigned int numpoints, double * obsPoint):
    ...
    try:
        if cdg == NULL or v == NULL or vt1 == NULL or vt2 == NULL or vt3 == NULL:
            raise MemoryError()
...
cdef list getShifted(double * wirePoint, int * indexes, unsigned int numpoints, double value):
    ...
    try:
        if vt1 == NULL or vt2 == NULL or vt3 == NULL or vt4 == NULL or obs == NULL or cdg == NULL:
            raise MemoryError()
        getCDG(cdg, wirePoint, indexes, numpoints)
        vlist = []
        if numpoints <= 2:
            for i in range(2):
                vlist.append(
                    Vector(wirePoint[indexes[i] * 3], wirePoint[indexes[i] * 3 + 1], wirePoint[indexes[i] * 3 + 2]))
            return vlist
        for j in range(3):
            obs[j] = 0
        for i in range(<unsigned int> (numpoints - 1)):
...
cdef list getIntersectionPointWithWire(double * wirePoint, int * indexes, unsigned int numpoints,
    ...
    try:
        if ipoint == NULL or tpoint == NULL or dirMIn == NULL or dirMOut == NULL or dirOIn == NULL or dirOOut == NULL:
            raise MemoryError()
        cortes = []
        for i in range(<int> numpoints - 1):
            for j in range(<int> otherNumpoints - 1):
                if not AlgSegment.getIntersectionPointWithSegment(ipoint, &wirePoint[indexes[i] * 3],
                                                                  ...
                                                                  <bint> True) or any(
                    [AlgVector.isEqual(ipoint, (<Vector> v)._v) for v in cortes]):
                    continue
                if (incEdge or (not AlgVector.isEqual(ipoint, &wirePoint[indexes[i] * 3])
                    ...
                    newVect = Vector(ipoint[0], ipoint[1], ipoint[2])
...
cdef list getIntersectionPointWithLine(double * wirePoint, int * indexes, unsigned int numpoints, double * linePoint,
                                       double * lineDir,
                                       bint incEdge):
...
cdef Wire getSubWire(double * wirePoint, int * indexes, unsigned int numpoints, double length1, double length2):
    ...
    for i in range(<int> numpoints - 1):
        ...
        _l += elength
    if not puntos:
        raise RuntimeError("Not founded subwire")
    if revert:
...
cdef void getNormal(double * normal, double * wirePoint, int * indexes, unsigned int numpoints):
    ...
    try:
        if cdg == NULL or vtemp1 == NULL or vtemp2 == NULL or vtemp3 == NULL:
            raise MemoryError()
        getCDG(cdg, wirePoint, indexes, numpoints)
...
cdef class IndexPerimeter():
    ...
    def __dealloc__(self):
        if self._m:
            # print("Borrado de indexperimeter")
            PyMem_Free(self._m)
            self._m = NULL
    ...
    cpdef void setList(IndexPerimeter self, list cdata):
        cdef int i
        if self._m != NULL:
            self._m = <int *> PyMem_Realloc(self._m, len(cdata) * sizeof(int))
            if not self._m:
                raise MemoryError()
        else:
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, slice):
            ...
            for i in range(start, stop, step):
                ...
                phv.set_ref_object(self)  # evitar que lo borren
                phlist.append(phv)
            return phlist
...
cdef class Wire:
    ...
    def __init__(self, pointList=[], **kwargs):
        ...
        if pointList:
            if isinstance(pointList, (list, tuple)):
                ...
                # point_index = list(range(len(pointList)))
            elif isinstance(pointList, AlgMatrix.Matrix):
                self.vectMat._m = <double *> PyMem_Malloc(
                    (<AlgMatrix.Matrix> pointList)._rows * (<AlgMatrix.Matrix> pointList)._cols * sizeof(double))
    ...
    def __json__(self):
        return {'__jsoncls__': 'CLinAlg.AlgWire:Wire.from_JSON', 'mat': self.vectMat, 'indPer': self.indPer}

    @classmethod
    def from_JSON(cls, jsondict):
        obj = Wire(jsondict['mat'], indPer=jsondict['indPer'])
        return obj
    ...
    cpdef list getBounds(self):
        cdef list bound = [sys.float_info.max, -sys.float_info.max, sys.float_info.max, -sys.float_info.max,
    ...
    def getIntersectionPoint(self, other, incEdge=False):
        if isinstance(other, Wire):
            return getIntersectionPointWithWire(self.vectMat._m, self.indPer._m, self.indPer._npoint,
                                                (<Wire> other).vectMat._m, (<Wire> other).indPer._m,
                                                (<Wire> other).indPer._npoint,
                                                <bint> incEdge)
            ...
            return getIntersectionPointWithSegment(self.vectMat._m, self.indPer._m, self.indPer._npoint,
                                                   (<AlgSegment.Segment> other)._vini,
                                                   (<AlgSegment.Segment> other)._vfin,
                                                   <bint> incEdge)
        else:
